...
def calculate_total_cost_batch(price_file, usage_folder, merged_folder, summary_output_file):
    ...
    # Process all usage files in the folder that do not contain 'LZ_WEST'
    usage_files = glob.glob(os.path.join(usage_folder, "IntervalData*.csv"))
    for usage_file in usage_files:
        if "LZ_WEST" in usage_file:
            continue
...
# Example usage
calculate_total_cost_batch(
    price_file="0706LZ_HOUSTON.csv",
    usage_folder=".",  # folder containing usage CSVs
    merged_folder="merged_outputs",  # where to save merged files
    summary_output_file="summary_total_cost_LZ_HOUSTON.csv"
)